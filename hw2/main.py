...
def parse_args():
    ...
    p.add_argument("--data_fraction", type=float, default=0.5,
...
class DigitDataset(Dataset):
    def __init__(self, img_dir, ann_file, img_size=800, is_train=True,
        ...
        if is_train and data_fraction < 1.0:
            ...
            print(f"[DataFraction] Using {n}/{len(coco['images'])} training images ({data_fraction:.0%})")
    ...
    def _load_single(self, idx):
        ...
        if self.is_train:
            ...
            img = img.resize((max(1, int(img.width * sf)), max(1, int(img.height * sf))), Image.BILINEAR)
    ...
    def __getitem__(self, idx):
        ...
        # ---- Normal loading ----
        img, boxes, labels, info, scale, pad_left, pad_top, orig_w, orig_h = self._load_single(idx)
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img, scale, pad_left, pad_top = resize_with_pad(img, self.img_size)
        img = self.normalize(img)
        return img, int(os.path.splitext(fname)[0]), orig_h, orig_w, scale, pad_left, pad_top
...
def collate_test(batch):
    ...
    return torch.stack(imgs), list(ids), list(hs), list(ws), list(scales), list(pls), list(pts)
...
def coords_to_orig(boxes_cxcywh, img_size, scale, pad_left, pad_top, orig_w, orig_h):
    ...
    return torch.stack([x1, y1, (x2-x1).clamp(0), (y2-y1).clamp(0)], dim=-1)
...
class MSDeformableAttention(nn.Module):
    ...
    def forward(self, query, ref_points, value, spatial_shapes):
        ...
        weights = self.attn_weights(query).view(B, Q, self.n_heads, self.n_levels * self.n_points)
        weights = F.softmax(weights, dim=-1).view(B, Q, self.n_heads, self.n_levels, self.n_points)
        ref     = ref_points[:, :, None, :, None, :]
        spatial = torch.tensor([[w, h] for h, w in spatial_shapes], dtype=torch.float32, device=query.device)
        ...
        for lid, (h, w) in enumerate(spatial_shapes):
            v_l = v_list[lid].permute(0, 2, 3, 1).reshape(B * self.n_heads, self.head_dim, h, w)
            g_l = grids[:, :, :, lid, :, :].permute(0, 2, 1, 3, 4).reshape(B * self.n_heads, Q, self.n_points, 2)
            s_l = F.grid_sample(v_l, g_l, mode="bilinear", padding_mode="zeros", align_corners=False)
            sampled.append(s_l)
        sampled = torch.cat(sampled, dim=-1)
        w_flat  = weights.permute(0, 2, 1, 3, 4).reshape(B * self.n_heads, 1, Q, self.n_levels * self.n_points)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, memory, ref_points, spatial_shapes, attn_mask=None):
        q = k = tgt + query_pos
        tgt2, _ = self.self_attn(q, k, tgt, attn_mask=attn_mask)
        tgt  = self.norm1(tgt + self.dropout1(tgt2))
        tgt2 = self.cross_attn(tgt + query_pos, ref_points, memory, spatial_shapes)
...
class ResNet50Backbone(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.layer0 = nn.Sequential(bb.conv1, bb.bn1, bb.relu, bb.maxpool, bb.layer1)
    ...
    for b, target in enumerate(targets):
        ...
        for rep in range(dn_number):
            ...
            if box_noise_scale > 0:
                ...
                noisy_boxes = (noisy_boxes + rand_sign * rand_mag * box_wh * box_noise_scale).clamp(0.0, 1.0)
...
class DeformableDETR(nn.Module):
    def __init__(self, num_classes=10, hidden_dim=256, nheads=8,
        ...
        self.encoder = nn.ModuleList([
            EncoderLayer(hidden_dim, nheads, self.n_levels, n_points, dim_feedforward, dropout)
            for _ in range(enc_layers)
        ])
        self.decoder = nn.ModuleList([
            DecoderLayer(hidden_dim, nheads, self.n_levels, n_points, dim_feedforward, dropout)
        ...
        self.dn_qpos   = nn.Sequential(nn.Linear(4, hidden_dim), nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim))
    ...
    def _encoder_ref_points(self, spatial_shapes, device):
        ...
        for h, w in spatial_shapes:
            ...
            refs.append(torch.stack([ref_x.flatten(), ref_y.flatten()], dim=-1))
    ...
    def _build_dn_inputs(self, targets, device):
        ...
        out = make_denoising_queries(targets, self.num_classes, self.dn_number,
    ...
    def forward(self, x, targets=None):
        ...
        enc_ref = self._encoder_ref_points(spatial_shapes, device).unsqueeze(0).expand(B, -1, -1, -1)
        memory  = src_flat
        ...
        for lid, layer in enumerate(self.decoder):
            ...
            tgt          = layer(tgt, query_pos, memory, ref_for_attn, spatial_shapes, attn_mask=total_attn_mask)
            tgt_norm     = self.dec_norm(tgt)
            logits       = self.class_heads[lid](tgt_norm)
            pred_boxes   = (self.bbox_heads[lid](tgt_norm) + inverse_sigmoid(ref_pts)).sigmoid()
    ...
    def forward(self, outputs, targets, use_focal=False):
        ...
        for b in range(B):
            ...
            if use_focal:
                ...
                cost_cls  = pos_cost[:, tgt_labels - 1] - neg_cost[:, tgt_labels - 1]
            else:
                cost_cls = -outputs["pred_logits"][b].softmax(-1)[:, tgt_labels]
...
class SetCriterion(nn.Module):
    ...
    def _compute(self, outputs, targets, indices):
        ...
        if self.use_focal:
            ...
            loss_ce = sigmoid_focal_loss(outputs["pred_logits"], tgt_oh,
                                         self.focal_alpha, self.focal_gamma, num_boxes)
        else:
            B, Q, _ = outputs["pred_logits"].shape
            tgt_classes = torch.zeros(B, Q, dtype=torch.long, device=device)
            for b, (src_idx, tgt_idx) in enumerate(indices):
                if len(src_idx):
                    tgt_classes[b, src_idx] = targets[b]["labels"][tgt_idx].to(device)
            loss_ce = F.cross_entropy(
                outputs["pred_logits"].permute(0, 2, 1), tgt_classes, weight=self.empty_weight
            )
        ...
        if src_boxes:
            sb = torch.cat(src_boxes);  tb = torch.cat(tgt_boxes)
    ...
    def _compute_dn_loss(self, outputs, targets, dn_meta):
        ...
        if not src_l:
            return torch.tensor(0.0, device=device)
        src_b = torch.cat(src_b);  tgt_b = torch.cat(tgt_b)
        ...
        if self.use_focal:
            ...
            loss_ce = sigmoid_focal_loss(src_l_cat, tgt_oh, self.focal_alpha, self.focal_gamma, nb)
        else:
            src_l_cat = torch.cat(src_l);  tgt_l_cat = torch.cat(tgt_l)
            loss_ce   = F.cross_entropy(src_l_cat, tgt_l_cat, weight=self.empty_weight)
        ...
        return (self.w_ce * loss_ce + self.w_bbox * loss_bbox + self.w_giou * loss_giou) * self.dn_loss_coef
    ...
    def _split_match_outputs(self, outputs):
        ...
        if "aux_outputs" in outputs:
            match_out["aux_outputs"] = [
                {"pred_logits": a["pred_logits"][:, pad:], "pred_boxes": a["pred_boxes"][:, pad:]}
...
def postprocess_single(logits, boxes, img_size, scale, pad_left, pad_top,
    ...
    scores  = scores[keep];  cls_ids = cls_ids[keep];  boxes = boxes[keep]
    if scores.numel() == 0:
        return []

    bx_xywh = coords_to_orig(boxes.cpu(), img_size, scale, pad_left, pad_top, orig_w, orig_h)
    bx_xyxy = bx_xywh.clone()
    bx_xyxy[:, 2] += bx_xyxy[:, 0];  bx_xyxy[:, 3] += bx_xyxy[:, 1]

    ki = []
    for cls in cls_ids.unique():
        cm = (cls_ids == cls).nonzero(as_tuple=True)[0]
        ki.append(cm[nms(bx_xyxy[cm].float(), scores[cm].float(), nms_thresh)])
    if not ki:
        return []
    ki = torch.cat(ki)
    return [{"bbox": bx_xywh[i].tolist(), "score": float(scores[i]), "category_id": int(cls_ids[i])}
            for i in ki.tolist()]
...
def train_one_epoch(model, criterion, loader, optimizer, device, scaler, ema, use_dn, clip_max_norm):
    model.train()
    criterion.train()
    total = 0.0
    nb    = 0
...
def evaluate(model, criterion, loader, device, img_size, output_dir, epoch,
    ...
    for imgs, targets in tqdm(loader, desc="Eval", leave=False):
        ...
        for b, (src_idx, tgt_idx) in enumerate(indices):
            if len(src_idx) > 0:
                ...
                all_gt_cls.extend(targets[b]["labels"][tgt_idx].cpu().tolist())
    ...
    acc = (sum(p == g for p, g in zip(all_pred_cls, all_gt_cls)) / len(all_pred_cls)
           if all_pred_cls else 0.0)
...
def plot_curves(history, output_dir):
    ...
    if vm:
        axes[1, 1].plot(em, vm, marker="o", markersize=3, label="mAP @[.5:.95]")
...
def plot_confusion_matrix(pred_cls, gt_cls, output_dir, epoch, num_classes=10):
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"confusion_matrix_ep{epoch}.png"), dpi=150)
        plt.close(fig)
    except ImportError:
...
@torch.no_grad()
def run_inference(model, test_dir, img_size, device, score_thresh, nms_thresh, use_focal):
    model.eval()
    ds     = TestDataset(test_dir, img_size)
    loader = DataLoader(ds, batch_size=2, shuffle=False, num_workers=4, collate_fn=collate_test,
    ...
    for imgs, img_ids, hs, ws, scales, pls, pts in tqdm(loader, desc="Inference"):
        ...
        for i in range(imgs.shape[0]):
            preds = postprocess_single(
                outputs["pred_logits"][i].cpu(), outputs["pred_boxes"][i].cpu(),
                img_size, scales[i], pls[i], pts[i], ws[i], hs[i],
...
def main():
    ...
    print(f"Data fraction={args.data_fraction} | batch_size={args.batch_size}")
